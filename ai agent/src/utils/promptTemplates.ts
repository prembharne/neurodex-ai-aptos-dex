...
export function buildUserPrompt(userMessage: string): string {
  const examples = FEW_SHOT_EXAMPLES.map(
    (ex, i) => `Example ${i + 1} User: ${ex.user}\nExample ${i + 1} JSON: ${JSON.stringify(ex.json)}`
  ).join('\n');
...
export function suggestClarification(errors: string[]): ClarificationRequest {
  if (!errors.length) return { needsClarification: false };
