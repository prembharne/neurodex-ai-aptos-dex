...
export function safeParseIntent(data) {
    return Intent.safeParse(data);
}
export function isTransferIntent(i) {
    return i.type === 'TRANSFER';
}
export function isSwapIntent(i) {
    return i.type === 'SWAP';
}
export function isUnknownIntent(i) {
    return i.type === 'UNKNOWN';
}
export function isRiskQueryIntent(i) { return i.type === 'RISK_QUERY'; }
...
export function toUnknownIntent(partial) {
...
export function classifyLooseIntent(input) {
    const result = safeParseIntent(input);
    if (result.success)
        return validateIntent(result.data);
    ...
    if (intent.type === 'SWAP') {
        ...
        if (issues.length) {
            const err = new z.ZodError([
                ...issues.map((m) => ({ code: z.ZodIssueCode.custom, message: m, path: ['SWAP'] })),
            ]);
            throw err;
        }
    }
